...
from constants import LAUNCH_CLEARANCE, MAX_FLEETS, FLEET_CAP_PER_PLAYER, N_PLAYERS, SUN_X, SUN_Y, N_COMET_SLOTS
...
def step(state: JaxState, launch, angle, ships, target=None, arrival=None) -> JaxState:
    ...
    # per-player IN-FLIGHT FLEET CAP (TRAIN-ONLY regularizer; eval/deploy rl_infer uncapped; match 2p exp25, generalized
    # to N_PLAYERS). A player already holding FLEET_CAP_PER_PLAYER in-flight has further launches REJECTED this tick; a
    # multi-launch tick crossing the cap drops the excess (planet-slot order, per-OWNER cumsum rank). Each player's 128
    # is INDEPENDENT (A's existing/rank never affect B's). Bounds total in-flight <= N*cap = MAX_FLEETS, so the shared
    # free-slot scatter below NEVER overflows/drops. (state.f_* here = the CURRENT in-flight set, pre-launch.)
    _cap_ok = jnp.ones((P,), bool)
    for _k in range(N_PLAYERS):
        _exk = jnp.sum(state.f_mask & (state.f_owner == _k))                          # owner k's current in-flight count
        _rankk = jnp.cumsum((valid & (state.p_owner == _k)).astype(jnp.int32)) - 1    # 0-idx rank among k's valids this tick
        _cap_ok = jnp.where(state.p_owner == _k, (_exk + _rankk) < FLEET_CAP_PER_PLAYER, _cap_ok)
    valid = valid & _cap_ok
    sp = fleet_speed_jax(ships)
    ...
    # ---- 7. combat: aggregate arrivals per planet (by owner), resolve N-WAY (4p, match v12-4p) ----
    # Exact 4-player generalization of forward_sim._resolve_combat: group arriving ships by owner, compare
    # LARGEST vs SECOND-largest. Survivor = top_owner with (top-second) ships; a TIE at the top destroys ALL
    # attackers. Reduces exactly to 2p. (FIX 2026-06-19: was 2-way inc0/inc1 -> owners 2,3 were silently
    # dropped from combat + owner 0 won ties; that crippled seats 2,3 in self-play and biased a1st to seat 0.)
    arrived = (hit_planet >= 0)
    oh = (hit_planet[:, None] == ar_P[None, :]) & arrived[:, None]   # (F,P)
    NP = 4
    inc = jnp.stack([jnp.sum(oh * ((f_owner == k) * f_ships)[:, None], axis=0)   # (NP,P) ships per owner
                     for k in range(NP)], axis=0)
    desc = -jnp.sort(-inc, axis=0)                                              # (NP,P) descending per planet
    top = desc[0]; second = desc[1]                                            # (P,) largest / 2nd-largest
    top_owner = jnp.argmax(inc, axis=0).astype(jnp.int32)                       # first owner holding the max
    has_surv = (top > 0) & (top > second)                                      # strict winner; tie destroys all
    surv_owner = jnp.where(has_surv, top_owner, -1).astype(jnp.int32)
    surv_ships = jnp.where(has_surv, top - second, 0)
