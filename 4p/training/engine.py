...
# --- SELF-CONTAINED for Kaggle (2026-06-14): import the physics from the FROZEN local copy
# (utils/physics = a copy of shared/physics), NOT shared.physics 4 levels up. A bare Kaggle
# submission bundle is just this exp folder, so an out-of-folder `from shared.physics` import
# would die at agent load. _HERE (this folder) on sys.path -> `utils.physics` resolves locally.
...
class AgentState:
    ...
    def _compute_fleet_hit(self, f, planets, comets, comet_pids, step, ptable):
        # walk a 100-turn horizon to MATCH the env's predict_first_hits (H_MAX_FIRSTHIT=100), NOT to
        # turn 499 (2026-06-14): a 449-turn walk would stamp f_target for long-flight fleets the env
        # records as -1 (>100 turns out), feeding a forecast-feature the policy never trained on.
        pid, rel, kind = first_hit_from(
            f[F_X], f[F_Y], f[F_ANGLE], f[F_SHIPS], planets, self.av,
            comets=comets, comet_planet_ids=comet_pids,
            start_turn=step, end_turn=min(LAST_TURN, step + 100), planet_table=ptable)
