...
def _fleet_movement_njit(
    ...
    # ISOLATED BUNDLE: this native-C helper backs ONLY the retired 1.28.0 two-phase
    # collision model (_move_collide_two_phase), which is unreachable — step() always
    # uses the pure-Python 1.30.1 swept path. The shared.agent_lib.v9 dep is dropped.
    raise RuntimeError("native-C 1.28.0 collision path is retired/unreachable in this bundle")
    n = fx_arr.shape[0]
...
def _sweep_collisions_njit(
    ...
    import ctypes as _ct
    # ISOLATED BUNDLE: retired 1.28.0 two-phase helper, unreachable (see _fleet_movement_njit).
    raise RuntimeError("native-C 1.28.0 collision path is retired/unreachable in this bundle")
    n = fx_arr.shape[0]
